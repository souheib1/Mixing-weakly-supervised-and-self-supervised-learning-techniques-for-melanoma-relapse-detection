...
def main():
    ...
    for epoch in range(1,num_epochs+1):
        ...
        for slide_folder in train_slides:
            ...
            with open(os.path.join(PATH_TRAIN, slide_folder), 'r') as f:
                reader = csv.reader(f)
                for row in reader:
                    embedding = torch.tensor(np.array(row, dtype=np.float32)).to(device) # just use dataframe with shuffle
    ...
    torch.save(mil_network.state_dict(), './mil_network_weights.pth')
    ...
    with open('./accuracy_val.pkl', 'wb') as file:
